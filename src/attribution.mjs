...
const roleIndex = { 'azpr-check': 0, 'azpr-functional': 1, 'azpr-failure': 2, 'azpr-deep': 3, 'azpr-verify-free': 4, 'azpr-verify-paid': 4 };
export function reviewProvenance(run) {
  return { mode: run.mode, stages: run.stages.filter(s => s.status !== 'FAILED' && roleIndex[s.role] !== undefined).sort((a, b) => roleIndex[a.role] - roleIndex[b.role]).map(s => ({ role: s.role, model: s.model, findings: s.result?.findings?.length })) };
}
export function provenanceReport(provenance, final, language) {
  const w = words(language);
  const rows = provenance.stages.map(s => `| ${w.roles[roleIndex[s.role]]} | \`${s.model}\` | ${s.findings ?? '—'} |`).join('\n');
...
export function commentAttribution(provenance, language, commentModel) {
  ...
  const w = words(language);
  const stages = provenance.stages.map(s => `${w.roles[roleIndex[s.role]]}: \`${s.model}\``).join('; ');
  return `${w.ai} — ${stages}; ${w.comments}: \`${commentModel}\`.\n${w.method}\n${w.posted}`;
}
