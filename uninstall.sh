...
if [ "$apply" -eq 0 ]; then
  printf 'Preview: archive azpr/, five pr-* commands, and plugins/azpr.js from:\n%s\nRe-run with --apply. No original models, settings, MCP, agents or other plugins are changed.\n' "$root"
  exit 0
fi
mkdir -p "$root/azpr-backups"
backup=$(mktemp -d "$root/azpr-backups/uninstalled.XXXXXX")
for rel in azpr commands/pr-check.md commands/pr-review.md commands/pr-deep.md commands/pr-stop.md commands/pr-comment.md plugins/azpr.js; do
