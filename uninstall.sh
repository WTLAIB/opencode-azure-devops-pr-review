...
for sub in commands plugins plugins/azpr azpr azpr-backups; do [ ! -L "$root/$sub" ] || { printf 'Resolve symlink manually: %s\n' "$sub" >&2; exit 1; }; done
...
for spec in 'plugins/azpr.js|azpr-optin:plugin' 'plugins/azpr/runtime.mjs|AZPR opt-in OpenCode adapter' 'azpr/runtime.mjs|AZPR opt-in OpenCode adapter'; do
  f=$root/${spec%%|*}; marker=${spec#*|}
  if [ -e "$f" ] && ! grep -Fq "$marker" "$f"; then printf 'Not owned; refusing: %s\n' "$f" >&2; exit 1; fi
done
if [ "$apply" -eq 0 ]; then
  printf 'Preview: archive plugins/azpr/, any old azpr/, five pr-* commands, and plugins/azpr.js from:\n%s\nRe-run with --apply. No original models, settings, MCP, agents or other plugins are changed.\n' "$root"
  exit 0
fi
mkdir -p "$root/azpr-backups"
backup=$(mktemp -d "$root/azpr-backups/uninstalled.XXXXXX")
for rel in azpr plugins/azpr commands/pr-check.md commands/pr-review.md commands/pr-deep.md commands/pr-stop.md commands/pr-comment.md plugins/azpr.js; do
